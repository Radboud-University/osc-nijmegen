...
cols = c("data_sharing", "code_sharing", "resource_sharing")
long <- select(dat, career_stage, cols) %>%
        gather(component, value, -c(career_stage))
long$value <- factor(long$value, levels = ordered_levels)
...
ggplot(data = long, mapping = aes(x = value)) +
  ...
  theme_classic() +
  scale_x_discrete(labels = new_xmarks) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# 
cols = c("open_access", "open_review", "open_edu", "alt_eval")
long <- select(dat, career_stage, cols) %>%
        gather(component, value, -c(career_stage))
long$value <- factor(long$value, levels = ordered_levels)
